...
public class TFTPSim {
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			if (command.equals("help")) {
				...
				tftpSim.errorCommand = ErrorCommands.NORMAL;
					...
					.equalsIgnoreCase(ErrorCommands.ERROR_CHANGE_OPCODE
							.toString())) {
				tftpSim.errorCommand = ErrorCommands.ERROR_CHANGE_OPCODE;
			} else if (command
					.equalsIgnoreCase(ErrorCommands.ERROR_CHANGE_BLOCK_NUM
							.toString())) {
				tftpSim.errorCommand = ErrorCommands.ERROR_CHANGE_BLOCK_NUM;

			} else if (command
					.equalsIgnoreCase(ErrorCommands.ERROR_REMOVE_FILENAME_DELIMITER
							.toString())) {
				tftpSim.errorCommand = ErrorCommands.ERROR_REMOVE_FILENAME_DELIMITER;
			} else if (command
					.equalsIgnoreCase(ErrorCommands.ERROR_REMOVE_MODE_DELIMITER
							.toString())) {
				tftpSim.errorCommand = ErrorCommands.ERROR_REMOVE_MODE_DELIMITER;
			} else if (command.equalsIgnoreCase(ErrorCommands.ERROR_MODIFY_MODE
					.toString())) {
				tftpSim.errorCommand = ErrorCommands.ERROR_MODIFY_MODE;
				...
				while (!isValid) {
					...
					if (command
							.equalsIgnoreCase(ErrorCommands.ERROR_APPEND_DATA
									.toString())) {
						tftpSim.packetType = PacketType.DATA;
						tftpSim.errorCommand = ErrorCommands.ERROR_APPEND_DATA;
						isValid = true;
					} else if (command
							.equalsIgnoreCase(ErrorCommands.ERROR_APPEND_ACK
									.toString())) {
						tftpSim.packetType = PacketType.ACK;
						tftpSim.errorCommand = ErrorCommands.ERROR_APPEND_ACK;
						isValid = true;
					} else {
				...
				while (!isValid) {
					...
					if (command
							.equalsIgnoreCase(ErrorCommands.ERROR_SHRINK_DATA
									.toString())) {
						tftpSim.packetType = PacketType.DATA;
						tftpSim.errorCommand = ErrorCommands.ERROR_SHRINK_DATA;
						isValid = true;
					} else if (command
							.equalsIgnoreCase(ErrorCommands.ERROR_SHRINK_ACK
									.toString())) {
						tftpSim.packetType = PacketType.ACK;
						tftpSim.errorCommand = ErrorCommands.ERROR_SHRINK_ACK;
						isValid = true;
					} else {
						System.out.println("Invalid command");
					}
					...
					.equalsIgnoreCase(ErrorCommands.ERROR_REMOVE_FILENAME
							.toString())) {
				tftpSim.errorCommand = ErrorCommands.ERROR_REMOVE_FILENAME;
			} else if (command.equalsIgnoreCase(ErrorCommands.ERROR_INVALID_TID
					.toString())) {
				tftpSim.errorCommand = ErrorCommands.ERROR_INVALID_TID;
			} else if (command.equalsIgnoreCase(ErrorCommands.ERROR_LOSE_PACKET
					.toString())) {
				tftpSim.errorCommand = ErrorCommands.ERROR_LOSE_PACKET;
			} else if (command
					.equalsIgnoreCase(ErrorCommands.ERROR_DELAY_PACKET
							.toString())) {
				tftpSim.errorCommand = ErrorCommands.ERROR_DELAY_PACKET;
			} else if (command
					.equalsIgnoreCase(ErrorCommands.ERROR_DUPLICATE_PACKET
							.toString())) {
				tftpSim.errorCommand = ErrorCommands.ERROR_DUPLICATE_PACKET;
			} else {
				tftpSim.errorCommand = ErrorCommands.NORMAL;
				printHelp();
			}
			...
			// then show the menu
			if (tftpSim.errorCommand.extendMenu1()) {
				boolean isValid = false;
				while (!isValid) {
					isValid = true;
					System.out
					...
					if (command.equalsIgnoreCase(PacketType.REQUEST.toString())) {
						tftpSim.packetType = PacketType.REQUEST;
			...
			} else if (tftpSim.errorCommand.extendMenu2()) {
				boolean isValid = false;
				while (!isValid) {
					isValid = true;
					System.out
			...
			if (tftpSim.errorCommand.extendMenu3()
					&& tftpSim.packetType != PacketType.REQUEST) {
				tftpSim.blockNumber = chooseBlockNum("Choose the Block Number: ");
				if (tftpSim.errorCommand == ErrorCommands.ERROR_CHANGE_BLOCK_NUM)
					tftpSim.newBlocknum = chooseBlockNum("Change the Block Number to: ");
			}
		}
